...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        if len(legal_actions) == 0:
            return None
        best_action = None
        q_value_of_best_action = -float("inf")

        for action in legal_actions:
            q_value = self.getQValue(state, action)
            if q_value > q_value_of_best_action:
                best_action = action
                q_value_of_best_action = q_value
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        q_values = [feature_values[feature]*self.weights[feature] for feature in feature_values]
        return sum(q_values)
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        for feature in feature_values:
            self.weights[feature] += self.alpha * difference * feature_values[feature]
